...
package:
	...
	aws cloudformation package --template-file sam.yaml --s3-bucket ${S3_BUCKET} --output-template-file build/sam.yml
	aws s3 cp build/sam.yml 's3://${S3_BUCKET}/sam.yml'

deploy:
	aws cloudformation deploy --template-file build/sam.yml --region ${AWS_REGION} \
